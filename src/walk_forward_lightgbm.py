...
# Explicitly exclude metadata, current-match statistics, and target variables
NON_FEATURE_COLS = [
    "Date",
    "Season",
    "HomeTeam",
    ...
    "ResultNumeric",
    "MatchID",
    "HomeXG",  # Exclude current match xG
    "AwayXG",  # Exclude current match xG
    "HS",
    "AS",
    "HST",
    "AST",
...
def main():
    ...
    # Filter strictly pre-match engineered features
    feature_cols = [c for c in df.columns if c not in NON_FEATURE_COLS]
    print(f"Total valid pre-match features selected: {len(feature_cols)}")
    ...
    print("\nRunning leakage-free Walk-Forward LightGBM evaluation...\n")
    ...
    print("\n" + "=" * 60)
    print("WALK-FORWARD LIGHTGBM OVERALL RESULTS (LEAK-FREE)")
    print("=" * 60)
    print(f"Matches evaluated: {len(pred_df)}")
    print(f"LightGBM Accuracy: {acc:.4f}")
    print(f"LightGBM Log Loss: {loss:.4f}  |  B365 Log Loss: {b365_loss:.4f}")
    print(f"LightGBM RPS:      {rps:.4f}  |  B365 RPS:      {b365_rps:.4f}")
